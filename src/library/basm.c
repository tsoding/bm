...
{
    if (type_stack->size < BM_STACK_CAPACITY) {
        type_stack->types[type_stack->size++] = type;
        return true;
    }
...
bool type_stack_pop(Type_Stack *type_stack, Type *output)
{
    if (type_stack->size > 0) {
        Type frame = type_stack->types[--type_stack->size];
        if (output) {
            *output = frame;
        }
        return true;
...
void basm_verify_program(Basm *basm)
{
    ...
    while (!halt) {
        ...
        case INST_DUP: {
            ...
            if (stack_addr < stack.size) {
                Type type = stack.types[stack.size - 1 - stack_addr];
                if (!type_stack_push(&stack, type)) {
                    fprintf(stderr, FL_Fmt": ERROR: stack overflow\n",
                            FL_Arg(basm->program_locations[ip]));
                    exit(1);
                }
        ...
        case INST_SWAP: {
            ...
            if (stack_addr < stack.size) {
                Type type = stack.types[stack.size - 1 - stack_addr];
                stack.types[stack.size - 1 - stack_addr] = stack.types[stack.size - 1];
                stack.types[stack.size - 1] = type;
            } else {
                fprintf(stderr, FL_Fmt": ERROR: stack underflow\n",
                        FL_Arg(basm->program_locations[ip]));
                exit(1);
        ...
        case INST_F2U: {
            for (size_t i = def.input.size; i > 0; --i) {
                Type actual_type = 0;
                if (!type_stack_pop(&stack, &actual_type)) {
                    fprintf(stderr, FL_Fmt": ERROR: stack underflow\n",
                            FL_Arg(basm->program_locations[ip]));
                    exit(1);
                }
            ...
            for (size_t i = 0; i < def.output.size; ++i) {
                if (!type_stack_push(&stack, def.output.types[i])) {
                    fprintf(stderr, FL_Fmt": ERROR: stack overflow\n",
                            FL_Arg(basm->program_locations[ip]));
                    exit(1);
                }
        ...
        case INST_RET:
        case INST_CALL:
            // TODO: verification for control-flow instructions is not implemented
        case INST_NATIVE:
